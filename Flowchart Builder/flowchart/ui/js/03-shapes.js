...
// ---------------------------------------------------------------------------
  ...
  function keyMark(kind) {               // the same shape, in miniature
    ...
    //
    // In the palette's colors from the start -- the ones paint() gives every
    // miniature (02-paint.js) -- rather than in white and near-black until
    // the chart is next painted.  Every list of them drawn again (the shapes
    // on the Style side after a pick, an undo, a new chart; the rules; the
    // menus) went back to the default palette until then: on the Night
    // palette, a row of white shapes that turned dark again a moment later.
    ...
    var art = kind === "text"
            ...
            : shapeArt(kind, 12, 8.5, 21, 14, fill);
    return '<span class="legendkey"><svg class="keymark" data-kind="' + kind +
           '" width="24" height="17" viewBox="0 0 24 17" stroke="' + line + '" ' +
