...
// ---------------------------------------------------------------------------
  ...
  var NAME_CAPS = /^(bmi|gpa|gcd|lcm|atm|id|pin|cpu|gpu|mph|kph|usa|uk|hw|cs|cis|csc|cse|io|ui|pc|tv|faq|diy|ok|rgb|html|css|sql|url|api|pdf|gps|dna|vat)$/;
  ...
  function tidyName(s) {
    ...
    if (!joined && !shout && /[A-Z]/.test(s)) { return shortTitle(s); }
    return shortTitle(s.split(" ").map(function (w, n) {
  ...
  // ---- what the comments at the top of a program call it -----------------
  // A comment at the top is where a program says what it is, but not every
  // comment there does.  "Title: sales_tax" and a short heading (// Tuition
  // Increase Calculator) name it.  "This program works out the sales tax"
  // says what it does -- which, said as a name, is "Works out the sales
  // tax" -- and so does a comment that starts with what it does (Calculate
  // the tax).  "Author: Pat Lee", "Date: 9/24" and "Name: ..." say who wrote
  // it, when and for which class; // Declare variables and // Get the input
  // are steps of it; and "Chapter 5 Exercise 7" is where it came from.  None
  // of those is a name.
  var NOTE_NAMED = /^(?:title|name of (?:the )?program|program(?: name)?|project(?: name)?|file(?: ?name)?)\s*[:=\-–—]\s*(.+)$/i;
  var NOTE_PURPOSE = /^(?:purpose|description|summary|about|goal|objective|task|what it does)\s*[:\-–—]\s*(.+)$/i;
  var NOTE_LABEL = /^[A-Za-z][A-Za-z.#']*(?: [A-Za-z.#']+){0,2}\s*:/;
  var NOTE_THIS = /^(?:(?:this|the|my)\s+(?:program|module|script|code|flowchart|pseudocode|algorithm|app)\s+(?:(?:will|should|is used to|is going to|is meant to)\s+)?|(?:a\s+)?(?:program|module|script|flowchart|pseudocode|algorithm)\s+(?:to|that|which)\s+)(?:(?:lets?|allows?|helps?)\s+(?:the\s+)?user\s+(?:to\s+)?)?(?=[a-z])/i;
  ...
  function noteTitle(s) {
    ...
    if (!s) { return null; }
    var m = NOTE_NAMED.exec(s);
    if (m) {
      var named = tidyName(m[1]);
      return nameWorth(named) ? { text: named, heading: true } : null;
    }
    m = NOTE_PURPOSE.exec(s);
    if (m) { return aboutNote(m[1]); }
    if (NOTE_LABEL.test(s)) { return null; }       // who, when, for which class
    if (NOTE_THIS.test(s)) { return aboutNote(s.replace(NOTE_THIS, "")); }
    ...
    var head = tidyName(s);
    if (s.split(" ").length <= 6 && !/[.?!,;]$/.test(s) && nameWorth(head) === 2) {
      return { text: head, heading: true };
    }
    return nameWorth(s) === 2 ? aboutNote(s) : null;
  }

  // What a comment says the program does, as far as its first full stop.
  function aboutNote(s) {
    s = String(s).split(/[.;!?](?:\s|$)/)[0].replace(/[\s.:,;!?\-]+$/, "").trim();
    return s ? { text: shortTitle(s.charAt(0).toUpperCase() + s.slice(1)), heading: false }
             : null;
  }

  // The top of a program, read for a name: the heading it gives itself, if
  // it gives one; what its comments say it does; and the first thing it
  // shows.  Only comments above the first Display count -- one further down
  // is about that part of it, not the whole -- and a /* block */ is read
  // line by line like the rest.
  function topNotes(code) {
    ...
    for (var i = 0; i < lines.length && i < 60; i++) {
      ...
      if (!got.said) {
        ...
        if (put && put[2].trim()) {
          got.said = shortTitle(put[2].replace(/[\s.:,;!?-]+$/, ""));
        }
      }
    }
    return got;
